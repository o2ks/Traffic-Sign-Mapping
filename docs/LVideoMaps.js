...
async function html5video(div_id='vplayer', video_link){
  ...
  async function YTvideo(div_id='vplayer', video_id, l, that){
    var vplayer = document.getElementById(div_id);
    vplayer.innerHTML = '';
  
    var video = document.createElement('div');
    video.setAttribute("id", "YTvideo");
    // video.setAttribute("style", player.getAttribute('style'));
    vplayer.appendChild(video);
  
    var player = new YT.Player('YTvideo', {
    //   height: '180',
    // width: '300',
    //   copy width, and height from vplayer height
      height: vplayer.offsetHeight-20,
      width: vplayer.offsetWidth-20,
      videoId: video_id,
      playerVars: {
        'playsinline': 1,
